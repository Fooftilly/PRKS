...
- **Credit-name override:** the credit on a Work-scoped Author row is the
  ...
  table is nevertheless required by the time `MERGE_WORKS` and
  `MOVE_MANIFESTATION` ship (Slice K)**, because both preserve colliding credit
  spellings there (§10.4).
...
| `MOVE_MANIFESTATION(M → Work)` | "This is really a Version of that Work." One `UPDATE manifestations SET work_id` re-keys everything the Manifestation owns by cascade (§4.1): its Assets' and annotations' `work_id`, its scoped roles, and its pinned argument sources. Every Argument that has a citation pinned to the moved Version gets its `argument-sources/<A>` revision **advanced in the same transaction**, exactly as `MERGE_WORKS` does, so a client holding the pre-move list conflicts instead of overwriting or re-pinning the moved citation. None of these can collide, because each identity includes the unchanged `MF-…` ID. If the Manifestation is a pointer target of the old Work and siblings remain, the pointer is re-pointed to a sibling first. If it is the old Work's **only** Manifestation, the command requires an explicit `empty_source` outcome, `delete` or `merge`, and retires the old Work in the same transaction (§4.1 "Transitions"). No placeholder Version is ever created. **Inherited metadata is frozen before the move**, by the same rule as `MERGE_WORKS` step 3. A NULL `title` or `abstract` override whose old-Work value differs from the destination's is written as an explicit override. The empty-source case is shown in the preview. **Credits:** for a person holding the same Work-scoped `(person, role_type)` on both Works under a different `credit_name`, the old spelling is frozen as a `manifestation_credit_overrides` row, by the same rule as `MERGE_WORKS` step 4 (an existing override is kept). The destination Work's *set* of Work-scoped creators does apply after the move, because moving a Version asserts that it belongs to that intellectual work. So Authors present on only one side cannot be frozen. The preview shows the Version's **credit delta** (added and removed creators). The preview lists the frozen values, the credit delta and the dropped relations. Title and abstract never change silently, and credit changes are always shown before the user confirms. Relations to Manifestations of the old Work are dropped, with a preview. | `MF-…` unchanged. |
...
| (new) `manifestation-field/[MF, title\|abstract]` for overrides | new scope, revision 0 until first written | Overrides are created by three paths: the Version-aware API, `MERGE_WORKS` step 3, and `MOVE_MANIFESTATION`, where both commands freeze inherited values (§10.4). All three advance `manifestation-field/[MF, title\|abstract]`. A **durable** legacy `SET_WORK_METADATA_FIELD(W, title\|abstract)` carries a base revision for `work-field/[W, f]`, which says nothing about the override. So while the primary Manifestation has an override for that field, the operation is **refused** with `FIELD_OVERRIDDEN_BY_VERSION` rather than routed. Online legacy `PATCH` follows §13.2 and advances the override's scope. |
...
**First Asset-bound edit with no origin Asset yet.** The server cannot see
operations still queued in a browser. So a live Work can reach Slice D with
no Asset-owned value, no Asset-bound revision and therefore no
`origin_AS(W)`, while a client still holds a queued first edit (for example a
`thumb_page` update at base revision 0). Such an operation creates the
placeholder `origin_AS(W)` **lazily, in the same transaction, just before
routing**. It uses the deterministic ID `AS-uuid5(…, "asset:" + W)` that the
backfill would have used, so the result is the same as if Slice A had
reserved it. The fresh scope starts at revision 0, so the base-0 edit applies
normally. Only a live, unmerged `W` gets this. A merged or deleted Work returns
its usual result.
