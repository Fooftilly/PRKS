/* Folder Tags UI. The TabContext owns the observed base and editor lifetime. */
(function (root) {
    'use strict';
    ...
    async function paint(ctx, state) {
        ...
        state.operations = rows.filter(o => o.entity_type === 'folder' && o.entity_id === state.folderId &&
            ['ADD_FOLDER_TAG', 'REMOVE_FOLDER_TAG'].includes(o.operation) && o.status !== 'acknowledged');
        if (!owns(ctx, state)) return;
        const panel = document.getElementById('panel-content');
        const list = panel.querySelector('#folder-panel-tags-list');
        if (!list) return;
        const work = ctx.getEntity('folder');
        const effective = { ...work, tags: root.prksEffectiveFolderTags(work, state.operations) };
        const editable = true;
        ...
        const chips = root.renderFolderTagsChipsHtml(effective, { editable });
        ...
        let status = panel.querySelector('[data-folder-tag-sync]');
        if (!status) {
            status = document.createElement('div'); status.dataset.folderTagSync = '';
        ...
        for (const op of state.operations.filter(o => o.status === 'conflict')) {
            ...
            const intent = (op.operation === 'ADD_FOLDER_TAG' ? 'Add ' : 'Remove ') + name;
            let message;
            ...
            else if (result.code === 'ENTITY_NOT_FOUND') message = 'This Folder or Tag no longer exists on the server.';
                ...
                button.onclick = async () => {
                    ...
                    try {
                        if (!apply && result.code === 'REVISION_CONFLICT') {
                            const ack = { folder_id: state.folderId, tag_id: op.payload.tag_id,
                            ...
                            const work = ctx.getEntity('folder');
                            root.prksOfflineMarkEntityChanged('folder', state.folderId);
                            root.prksOfflineMarkEntityChanged('folder-tag-options', state.folderId);
        ...
        const input = panel.querySelector('#folder-tag-search');
        if (input) {
    ...
    function acceptAck(ctx, state, ack) {
        if (!live(ctx, state) || ack.folder_id !== state.folderId) return;
        state.readVersion = (state.readVersion || 0) + 1;
        if (state.options && root.prksFolderTagBase(state.options, ack.tag_id).revision > ack.server_revision) return;
        const work = ctx.getEntity('folder');
        const tags = work.tags.filter(t => t.id !== ack.tag_id);
        if (ack.present) tags.push(ack.tag);
        ctx.setEntity('folder', { ...work, tags });
        if (state.options) {
    ...
    async function prepare(ctx, state) {
        ...
        try {
            const [options, catalog] = await Promise.allSettled([
                root.prksReadFolderTagOptions(state.folderId), root.prksReadTagsIndex(),
            ]);
    ...
    function bindPicker(ctx, state) {
        ...
        const results = document.getElementById('folder-tag-search-results');
        ...
        async function dropdown() {
            ...
            const tags = root.prksEffectiveTagChips
                ? root.prksEffectiveTagChips(
                    root.prksEffectiveFolderTags(ctx.getEntity('folder'), state.operations),
            ...
            if (value && !catalog.some(t => root.prksTagExactMatch(t, query))) {
                ...
                item('Create tag "' + value + '"',
                    () => root.prksSubmitNewTag('folder', state.folderId, value, ctx, input),
    ...
    async function edit(ctx, tagId, present, knownTag) {
        const state = ctx && ctx.getResource('folderTagEditor');
        ...
        try {
            ...
            const tag = knownTag || (state.catalog || []).find(t => t.id === tagId) ||
                ctx.getEntity('folder').tags.find(t => t.id === tagId) ||
    ...
    root.prksMountFolderTags = mount;
    root.prksFolderTagEdit = edit;
