 ...
 * the disposable cache. No module-level Folder or editor state. */
(function (root) {
    'use strict';
    ...
    function optionsShape(v, folderId) {
        if (!v || !id(v.folder_id) || (folderId && v.folder_id !== folderId) || !Array.isArray(v.assigned) ||
    ...
    async function readOptions(folderId, options = {}) {
        const result = await root.prksOfflineReadEntity('folder-tag-options', folderId,
            '/api/folders/' + encodeURIComponent(folderId) + '/tag-options', {
                ...options, validate: v => optionsShape(v, folderId),
            });
        if (result.value !== null && !optionsShape(result.value, folderId)) {
            await root.prksOfflineInvalidateEntity('folder-tag-options', folderId);
            return { value: null, source: 'unavailable', cachedAt: null };
        }
        return result;
    }
    ...
    function effectiveTags(folder, operations) {
        const tags = new Map((folder.tags || []).map(t => [t.id, t]));
        operations.filter(op => op.entity_id === folder.id && op.entity_type === 'folder' && op.status !== 'acknowledged')
            .forEach(op => {
                if (op.operation === 'REMOVE_FOLDER_TAG') tags.delete(op.payload.tag_id);
                if (op.operation === 'ADD_FOLDER_TAG' && op.local_context && op.local_context.tag) {
     ...
     * A Folder-Tag edit is something the user chose, so every terminal outcome
    ...
    function isResult(data, op) {
        if (!data || data.folder_id !== op.entity_id || data.tag_id !== op.payload.tag_id) return false;
        switch (data.code) {
            case 'ACKNOWLEDGED':
                return typeof data.present === 'boolean' && data.present === (op.operation === 'ADD_FOLDER_TAG') &&
            ...
            case 'REVISION_CONFLICT': case 'FUTURE_REVISION':
                return integer(data.current_revision) && typeof data.current_state === 'boolean' &&
                    data.requested_state === (op.operation === 'ADD_FOLDER_TAG');
    ...
    const handler = {
        isResult, terminal,
        reconcile: data => root.prksOfflineReconcileFolderTag(data),
    };
    Object.assign(root, {
        prksIsTagsIndexShape: tagsShape, prksIsFolderTagOptionsShape: optionsShape,
        prksReadTagsIndex: readTags, prksReadFolderTagOptions: readOptions,
        prksFolderTagBase: base, prksEffectiveFolderTags: effectiveTags,
        prksFolderTagSyncHandler: handler,
    });
