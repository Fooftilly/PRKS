...
class WorkDetailsPolishTests(_BrowserE2E):
    ...
    def test_details_modes_and_metadata_draft_survive_panel_tab_changes(self):
        ...
        self.assertEqual(page.locator("#meta-status").input_value(), "In Progress")
        self.assertEqual(page.locator("#meta-doc-type").input_value(), "book")
        page.locator('#panel-content .prks-segmented__btn[data-value="Completed"]').click()
        self.assertEqual(page.locator("#meta-status").input_value(), "Completed")
        page.locator("#meta-doc-type-trigger").click()
...
class PersonGroupPolishTests(_BrowserE2E):
    def test_member_mode_resets_when_navigating_to_another_group(self):
        _server, page, _collector = self._start_app()
        ...
        page.fill("#group-name", "E2E Members Group A")
        page.locator("#save-group-btn").click()
        page.wait_for_function("() => location.hash.indexOf('#/people/groups/') === 0")
        group_a = page.evaluate("() => location.hash.split('/')[3]")

        page.evaluate("() => window.prksNavigate('#/people/groups')")
        page.wait_for_function("() => location.hash === '#/people/groups'")
        page.locator(".prks-group-library__header .prks-btn", has_text="New group").click()
        page.locator("#group-modal:not(.hidden)").wait_for()
        page.fill("#group-name", "E2E Members Group B")
        page.locator("#save-group-btn").click()
        page.wait_for_function("() => location.hash.indexOf('#/people/groups/') === 0")
        group_b = page.evaluate("() => location.hash.split('/')[3]")

        page.evaluate("id => window.prksNavigate('#/people/groups/' + id)", group_a)
        page.wait_for_function("id => location.hash === '#/people/groups/' + id", arg=group_a)
        ...
        page.evaluate("id => window.prksNavigate('#/people/groups/' + id)", group_b)
        page.wait_for_function("id => location.hash === '#/people/groups/' + id", arg=group_b)
        self.assertEqual(page.locator("#group-add-member-search").count(), 0)
        page.locator(".group-detail__section-head button", has_text="Manage members").wait_for()
        self.assertFalse(page.evaluate("() => window.prksGetFocusedTabContext().ui.personGroupMembersEditing"))
...
class WorkspaceTabsTests(_BrowserE2E):
    ...
    def test_private_reminder_hide_show_prefers_unsettled_complete_draft(self):
        ...
        # Unsettled draft only: dirty editor, or draft overlay that still
        # differs from the acknowledged entity body. Rejects settled-only
        # entity text (CodeRabbit) and does not require transient "Drafting…"
        # (Codex). Park immediately after this resolves on dirty.
        page.wait_for_function(
            """(args) => {
                ...
                if (typeof window.prksPrivateNotesTextForEntity !== 'function') return false;
                const overlay = window.prksPrivateNotesTextForEntity('work', args.workId, '');
                if (overlay !== args.text) return false;
                const work = ctx.getEntity && ctx.getEntity('work');
                const ack = work && work.private_notes != null ? String(work.private_notes) : '';
                return overlay !== ack;
            }""",
