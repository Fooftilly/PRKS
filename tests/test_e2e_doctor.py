...
class AssessmentClassificationTests(unittest.TestCase):
    ...
    def test_shm_probe_failed_is_incomplete(self):
        # exists() OSError must not look like absent/inapplicable shm.
        facts = self._facts()
        facts["resources"]["shm"] = {
            "present": False,
            "probe_failed": True,
...
class DiskProbeTests(unittest.TestCase):
    def test_disk_usage_attribute_error_is_structured(self):
        # Patch shutil.disk_usage directly so the error path works on Windows
        # (where disk_usage does not call os.statvfs).
    ...
    def test_exists_oserror_marks_probe_failed(self):
        ...
        self.assertIsNone(result["avail_bytes"])
        self.assertTrue(
            "OSError" in result["error"] or "Permission" in result["error"],
            result["error"],
        )

    def test_collect_report_survives_disk_usage_failure(self):
