...
class FrontendRequestCoordinatorTests(unittest.TestCase):
    ...
    def test_route_generation_and_abort_controller_remain(self):
        app = _read(_APP)
        tab_ctx = _read(_TAB_CONTEXT)
        self.assertIn("const generation = ctx.beginRoute(route)", app)
        self.assertIn("const routeAbort = ctx.abortController", app)
        self.assertIn("return new AbortController();", tab_ctx)
