...
Ordinary global-tab switching warm-suspends an eligible PDF Work by moving its existing TabContext root into `#prks-tab-warm-parking`. Resume moves that same root into its visible tile, preserves viewer/editor state, requests a container resize only, and performs no route render or Work/PDF fetch. A Work in metadata-edit mode is not eligible: after leave approval it cold-unmounts so TabContext teardown discards the runtime draft. The warm cache uses deterministic LRU eviction with at most three parked PDF contexts. Explicit hide, narrow fallback, close/batch close, and teardown remain cold lifecycle operations; non-PDF pages never enter the warm cache. Warm state is ephemeral and is not workspace persistence.
...
Creating a Work is a short primary workflow with optional depth, not a database form. Production creation goes through `#work-modal` (`POST /api/works`, or durable `CREATE_WORK` for YouTube). Do not duplicate that path, and do not add a "quick add" API: essentials and enrichment are one submit.
...
**Essentials first:** The first screen holds only what establishes the Work: Source (`Required`; compact PDF/YouTube toggle beside the section title and a one-line drop target or the YouTube URL), Title, Folder (`Required`), Document type + Year on one row, and a one-line People adder. At a ~1280×800 viewport every essential and Create File are visible with no scrolling. Requiredness uses `.prks-field__required`, never placeholder text alone.

**Enrichment on demand:** Everything else sits in two `Optional` disclosures that start collapsed on every fresh open: **Publication details** (PDF: published date, original URL, publisher, location, edition, journal, volume, issue, pages, ISBN, DOI, thumbnail page; YouTube: channel, date published, accessed date) and **Tags, status & notes** (YouTube adds Playlist, and the summary says so). Do not remove fields; hide advanced ones until opened. Collapsed disclosure content is not keyboard-focusable (`inert`). A collapsed disclosure shows `N set` when it holds values, so nothing entered is silently out of sight. Opening/closing, switching sections, and validation never clear a value. An error on a field inside a collapsed disclosure opens it before focusing the field.

**People:** No role surface appears before there is a person. Picking a person from the search (pointer, or ArrowDown/Enter) adds them at once as Author — or the first role they do not already hold — as a compact row with its own role `<select>` (all New File roles), a `Name on file` control, and Remove. `Name on file` opens the one shared credit picker (aliases, override) under that row, so `credit_name` keeps its existing semantics. A duplicate person+role is refused inline at the row. Quick-create adds the new person the same way.

**Comboboxes:** Folder, People, Tags, and Playlist results support ArrowUp/ArrowDown and Enter; the first ArrowDown lands on a real match rather than a `Quick-create…`/`Create…` row. An open result list is its own Escape layer, like the doc-type menu: the first Escape closes the list, the next follows the modal lifecycle.
...
**Folder:** The default destination reads `Uncategorized`, matching what the server actually stores for an empty `folder_id` (`ensure_default_uncategorized_folder_id()`) — never `Library root`. Free text typed into the folder search is a query, not a selection; only an explicitly chosen result or the explicit default commits a destination. An uncommitted query shows its inline error when the field is left, blocks submission, and focuses the field. A destination that no longer exists is refused before anything is created (`FOLDER_NOT_FOUND`) and reported on the Folder field. New File inherits its folder context from the focused TabContext's route (`prksFocusedRouteRecord()`), not from `window.location.hash`; a focused pane that isn't a Folder defaults to Uncategorized rather than inheriting Main's folder.

**Actions:** Sticky modal footer keeps Cancel (secondary) and Create File (primary) visible while the body scrolls. The dialog has a stable height, so content that appears (a YouTube preview, a People row, an opened disclosure) scrolls the body rather than moving Create under the pointer. Create File is the only prominent submit; Enter in Title and Ctrl/Cmd+Enter anywhere in the dialog press it. Duplicate submit is blocked (`Creating…`). Recoverable validation stays inline (`aria-invalid`, `aria-describedby`), appears when an obviously invalid field is left (folder query, YouTube URL, dd/mm/yyyy dates), focuses the first invalid control on submit, and preserves entered metadata. A refused create says `Nothing was saved.` Success closes the modal and uses `prksNavigate` to the new Work; if tags fail to attach after the Work exists, the Work still opens and an alert names what was not attached, so a retry can never duplicate the Work.
