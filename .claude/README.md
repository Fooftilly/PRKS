...
1. Install the SonarQube CLI for your OS (Linux, macOS ARM64, or Windows).
2. Authenticate locally (`sonar auth login`). Tokens stay on the machine; they
   are never committed (`.mcp.json` is token-free).
3. Restart the terminal so `PATH` picks up `sonar`.

Without `sonar` on `PATH`:

- **Secrets hooks** (`.claude/settings.json`) no-op via
  `.claude/hooks/sonar-secrets/run-hook.mjs`.
...
secrets scans. Commands use Claude Code **exec form** (`node` + `args`) so the
same config works on Windows PowerShell, macOS, and Linux without Git Bash.
