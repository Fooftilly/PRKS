...
- Agent/dev E2E loop (preferred): local development may use `python tests/e2e/run.py --smoke`, `--feature <group>`, `--affected`, `--last-failed`, or `--dev --feature <group>`. Cloud coding agents should prefer `scripts/e2e agent`, which runs affected E2E fail-fast with pointer capture disabled and chooses a conservative worker count from cgroup CPU/memory limits (maximum 2 unless explicitly overridden). Never iterate on the full suite. Convenience: `scripts/e2e smoke|feature|affected|last-failed|dev|agent|full`. On-demand suite inventory (E2E by feature/module, timing weights, Node/Python counts): `scripts/e2e inventory`.
