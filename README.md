...
PRKS can be installed as a Progressive Web App and stays useful for a while when the PRKS server becomes temporarily unreachable. This is **read-only** support, and it currently covers **Work detail pages and their PDFs, the Concepts list and individually opened Concept pages, and the Positions list and individually opened Position pages**. Person, Argument/Stance, Person Group and Playlist pages, and the Research Graph, are not part of the offline cache yet and behave exactly as they do without any of this (a temporarily unreachable server surfaces their normal error state, not a cached read). Phase 1 also has no offline mutation queue.
...
- Navigating between cached Works, cached Concepts and cached Positions — parent/subconcept links, a Concept's research-note mentions back to a cached Work, the command palette, and links inside research notes — works exactly like online navigation (`prksNavigate`). There is no separate offline router.
...
- The app additionally keeps a small IndexedDB (`prks-offline-v1`) of the Work, Concept and Position records above, plus the cached Concepts and Positions lists, each stamped with when it was cached.
