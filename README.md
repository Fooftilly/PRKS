...
## Offline / PWA (read-only in Phase 1)

PRKS can be installed as a Progressive Web App and stays useful for a while when the PRKS server becomes temporarily unreachable. This is **read-only** support, and it currently covers **Work detail pages and their PDFs, the Concepts list and individually opened Concept pages, the Positions list and individually opened Position pages, the Arguments & Stances list and individually opened Argument or Stance pages, and the People list, its role views and individually opened Person profiles**. Person Group and Playlist pages, and the Research Graph, are not part of the offline cache yet and behave exactly as they do without any of this (a temporarily unreachable server surfaces their normal error state, not a cached read). Phase 1 also has no offline mutation queue.
...
- Navigating between cached Works, cached Concepts, cached Positions, cached Arguments and cached People — parent/subconcept links, a Concept's research-note mentions back to a cached Work, the command palette, and links inside research notes — works exactly like online navigation (`prksNavigate`). There is no separate offline router.
...
- A cached Position page still lists the Arguments and Stances that target it, but **opening one of them requires a connection** — Arguments and Stances are not cached yet, so activating a row says so instead of loading a broken page. The link keeps its real address, so you can still copy or inspect where it points.
...
- A cached Person page still shows the groups they belong to, but **opening a group requires a connection** — Person Groups are not cached yet, so activating a group chip says so instead of loading a broken page. The link keeps its real address.
- **View in graph** and the Research Graph itself require a connection, and say so rather than opening a graph page that cannot load.
- Search/browse offline is limited to the Works and Concepts already cached on this device — there is no offline full-text search.
...
- The app additionally keeps a small IndexedDB (`prks-offline-v1`) of the Work, Concept, Position, Argument/Stance and Person records above, plus the cached Concepts, Positions, Arguments and People lists, each stamped with when it was cached. Images are never stored there.
