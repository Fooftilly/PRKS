...
- Status and bibliographic details save separately, on purpose. One button that meant "the status and these twelve other fields, either half able to fail on its own" is not something anyone could reason about afterwards.
...
- The rest of that editor (title and document type) still saves on the PRKS server. While you are offline its **Save Changes** button is disabled and says so, rather than letting you type into a form that cannot save.
