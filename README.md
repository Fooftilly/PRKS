...
PRKS can be installed as a Progressive Web App and stays useful for a while when the PRKS server becomes temporarily unreachable. This is **read-only** support, and it currently covers **Work detail pages and their PDFs, the Concepts list and individually opened Concept pages, the Positions list and individually opened Position pages, and the Arguments & Stances list and individually opened Argument or Stance pages**. Person, Person Group and Playlist pages, and the Research Graph, are not part of the offline cache yet and behave exactly as they do without any of this (a temporarily unreachable server surfaces their normal error state, not a cached read). Phase 1 also has no offline mutation queue.
...
- Navigating between cached Works, cached Concepts, cached Positions and cached Arguments — parent/subconcept links, a Concept's research-note mentions back to a cached Work, the command palette, and links inside research notes — works exactly like online navigation (`prksNavigate`). There is no separate offline router.
...
- The app additionally keeps a small IndexedDB (`prks-offline-v1`) of the Work, Concept, Position and Argument/Stance records above, plus the cached Concepts, Positions and Arguments lists, each stamped with when it was cached.
...
Some cached pages depend on more than one record — a Concept page shows other Concepts' names and the titles of the Works that mention it, and a Position page shows the names, kinds and verdicts of the Arguments targeting it. So when something changes that could make any of them wrong, PRKS drops that whole cached set rather than show you a page it knows may be out of date. For Concepts that means any Concept edit, a saved research note, a deleted Work, or a Work title change from anywhere (including renaming a video inside a Playlist). For Positions it means any Position edit, or an Argument being created, renamed, retargeted or deleted. Arguments & Stances depend on the most: any Argument edit, a Position being renamed, a Work title change, a saved research note, a deleted Work, an author being added to or removed from a cited Work, or that author's name being changed. Those pages simply become unavailable offline until you next view them online.

The three sets are kept separately, so editing a Concept never costs you your cached Arguments, and changing an Argument's sources never costs you your cached Positions. This is all deliberately cautious: nothing on the server is affected, and PRKS keeps working normally while you are online.
