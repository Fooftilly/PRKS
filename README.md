...
- **Edit metadata** on a file you have opened online at least once has its own **Bibliographic details** section — edition, journal, volume, issue, pages, ISBN and DOI — with its own **Save bibliographic details** button. That button works whether or not PRKS is reachable, and it takes the same route either way.
- The rest of that editor (title, status, document type, year, publisher, abstract and so on) still saves on the PRKS server. While you are offline its **Save Changes** button is disabled and says so, rather than letting you type into a form that cannot save.
