...
PRKS can be installed as a Progressive Web App and stays useful when the PRKS server becomes temporarily unreachable. A **disposable read cache** covers **the Folder Library at PRKS's home route and individually opened Folders, Work detail pages and their PDFs, the Concepts list and individually opened Concept pages, the Positions list and individually opened Position pages, the Arguments & Stances list and individually opened Argument or Stance pages, the People list, its role views and individually opened Person profiles, the Person Groups hierarchy with individually opened group pages, the Playlists list with individually opened playlists, the Research Graph once a Graph variant has been opened online, and the browse pages — Progress, File types, Recently opened and Recently added**. Separately, ordinary content changes take **the same local-first path with or without a server**: the change is written to durable browser storage before PRKS says it is saved, shows up everywhere it belongs straight away, survives reloads and browser restarts, and is sent once when PRKS answers again. Today that includes durable creation/editing/deletion for **Concepts, Positions, Arguments and Stances**; the Tag vocabulary (create, delete, merge) and attaching or removing Tags on prepared Works and Folders; creating a YouTube/video Work and deleting a file; editing a file's metadata, progress and video identity; editing a file's **Research Notes** and **Work reminders**; linking people to files; creating and editing People, Person Groups, folders and playlists; and the record of which files you opened. Each is covered below, and `docs/local-first-rollout-status.md` keeps the exact score of what is durable and what still needs the server.
...
- Two playlist controls still need a connection, and both are searches over something no cache holds: the **Add video** search on a playlist's editor searches your whole library, and the **playlist picker** on a video's own page searches every playlist. Both simply freeze; everything they lead to works offline — see below. On a video's own page the playlist card still shows which playlist it belongs to, and starting a *new* editing session there is unavailable until PRKS answers again — if you already had it open, it stays open, Clear and New… stay live, and Done still works. If the connection drops while a playlist editor is open, **what you have typed is kept on screen**, and Cancel/Close still work.
...
Deleting a file, attaching or removing a folder or playlist, changing a folder's tags, creating a YouTube/video Work, and the other durable surfaces below are **not** in this list — they take the same local-first path online and offline.

What you can change offline — Arguments and Stances:

- **Creating an Argument or Stance works offline**, including Create Response and Create from Work. It receives its permanent `A-` id immediately. Initial sources and targets are part of that single construction, so a response is never briefly detached from its parent and a Work-created Argument is never briefly uncited.
- **Name, kind and main text are independent changes.** Changing only a name never rewrites or blocks on untouched main text. Changing kind moves the record between the Arguments and Stances tabs immediately.
- **Sources and targets are ordered whole-list decisions.** Reordering counts as a change. Position and Argument targets share one target list even though the server stores them in two tables.
- A new Argument can target a Position created offline, and another new Argument can target that first Argument. PRKS records those dependencies and sends the chain in order.
- **Deletion works offline as a tombstone.** The record stays hidden through transport failures. If PRKS refuses because a research note names it or another Argument targets it, it returns immediately and Diagnostics names the refusal.
- Editing an acknowledged Argument offline requires that its detail and synchronization revisions were prepared on this device. If the revision is unknown, PRKS refuses safely instead of guessing. A locally-created pending Argument needs no server base.
...
- **Merging two Tags works offline.** A merge is an identity transform (null base): the source Tag disappears into the target everywhere it appeared, and if the source is still named in a pending queue row the merge is refused locally rather than sent for PRKS to reject. Conflicts and refusals surface in **Settings → Diagnostics**.
...
- **For a video, you can change which video it is** — paste a different YouTube link and press Save video source. Different links to the same video are treated as the same video, so re-pasting a share link instead of a watch link changes nothing. While you are offline the new choice is remembered but the video itself cannot load until PRKS is reachable again; online, the new video plays immediately, before PRKS has been told. Save is refused only when this device has never prepared the Work's sync base (open the file online once) — not merely because you are offline.
...
What you can change offline — Research Notes and Work reminders:

- **Research Notes and Work reminders (the private notes on a file) work offline.** Each is one whole document with its own revision. Editing one never conflicts with the other. Folder private notes are durable too — they travel as `SET_FOLDER_FIELD` on the folder, not as Work reminders.
- Your change is written to durable browser storage **before** the UI says it is saved, and that storage is a different database from the disposable cache. It survives reloads, browser restarts, PRKS server restarts and **Clear offline cache**. Typing the same text that is already on the server is not a disagreement.
- Research Notes still create Concepts when they reach PRKS, the same way they do online. This device does not parse the note to update Concept, Argument or Graph pages itself — those pages refresh once PRKS has processed the note.

What you can change offline — creating a YouTube file and deleting a file:

- **Creating a YouTube / video Work works offline.** The new file gets a permanent `W-` id chosen on this device, so it is real immediately: it appears in the library, can carry people and a folder or playlist chosen at create time, and Tags selected in the New File dialog are attached as ordinary durable Tag operations ordered behind the creation. PDF binary upload stays connection-required until durable Blob storage exists.
- **Deleting a file works offline.** It disappears from every cached list and detail straight away and stays gone after a reload. Managed PDF cleanup on the server is replay-safe: an exact operation replay never deletes a PDF that another Work now shares.
...
- **Attaching or removing Tags on a folder works offline**, the same way Work Tags do. Open the folder's Tags panel online once so its relationship snapshot is prepared; without that base, the panel asks you to connect once rather than guessing. Creating a brand-new Tag from the picker is durable too and is ordered ahead of the attachment.
...
What you can change offline — playlists and the order of what is in them:

- **Creating a playlist works offline**, from the Playlists page, a video's own panel, or the New File flow. The new playlist gets a permanent id chosen on this device, so it is a real playlist immediately: it appears in the list, and you can put videos in it before PRKS has heard of it. Those later changes are ordered behind the creation automatically.
- **Renaming a playlist and editing its description or original URL** work offline, and each field is tracked separately — a rename here and a description edit elsewhere both survive.
- **Adding a video to a playlist works offline**, and so does taking it out and moving it to a different one. A video is in one playlist at a time, so all three are the same action with different answers, and putting it back where it started leaves nothing queued. The playlist shows on the video's own page and the video appears on the playlist's own page — provided this device has a card for it; a video PRKS has never described to you is not invented.
- **Reordering works offline**, and an order is treated as one decision rather than as each video's position racing separately. Drag twice before anything is sent and only the order you ended on is kept. If somebody else reordered the same playlist meanwhile, you are asked to decide between the two orders — because they are two whole orders, and quietly interleaving them would produce a third that neither of you chose.
- **Renaming a video from inside a playlist** works offline too — but that is a change to the *video*, not to the playlist, so it takes the same path as renaming it anywhere else.
- **Deleting a playlist works offline.** Unlike a folder, a playlist holding videos is not protected: the videos are not inside it in any real sense, and they survive it. It disappears from the list straight away and stays gone after a reload, but nothing on this device is destroyed — if PRKS refuses, it comes back. A playlist you created offline and then deleted before it was ever sent disappears completely.
- One number is deliberately left alone: the **item count** beside a playlist still shows what PRKS last said, for the same reason a folder's file count does — a move changes two playlists' counts and this device cannot know which playlist the video left. A playlist you created here is the exception, because it genuinely holds nothing.
...
- Hooks use platform-aware launchers (`run_hook.sh` / `run_hook.ps1`) that
  locate Python the same way PRKS does (`python3` on POSIX; `python` / `py -3`
  on Windows — never a bare Windows `python3` requirement), then call
  `run_hook.py`. If `sonar` or a usable interpreter is missing, the hooks exit 0
  and do nothing.
