...
PRKS can be installed as a Progressive Web App and stays useful for a while when the PRKS server becomes temporarily unreachable. This is **read-only** support, and it currently covers **Work detail pages and their PDFs, the Concepts list and individually opened Concept pages, the Positions list and individually opened Position pages, the Arguments & Stances list and individually opened Argument or Stance pages, the People list, its role views and individually opened Person profiles, the Person Groups hierarchy with individually opened group pages, and the Playlists list with individually opened playlists**. The Research Graph is not part of the offline cache yet and behaves exactly as it does without any of this (a temporarily unreachable server surfaces its normal error state, not a cached read). Phase 1 also has no offline mutation queue.
...
- The app additionally keeps a small IndexedDB (`prks-offline-v1`) of the Work, Concept, Position, Argument/Stance, Person, person-group and playlist records above, plus the cached Concepts, Positions, Arguments, People, Person Groups and Playlists lists, each stamped with when it was cached. Images — portraits, file thumbnails and video artwork included — are never stored there.
...
The six sets are kept separately, so editing a Concept never costs you your cached People, and changing group membership never costs you your cached Arguments or playlists. This is all deliberately cautious: nothing on the server is affected, and PRKS keeps working normally while you are online.
